// ****************************************************************************************************
// File name: edit-service-diagnosis.js
// Last modified on
// 
// ****************************************************************************************************
function ServiceDiagnosisEdit(params){
	// console.log(params)
	return new FormEditor({
		...
		url: params.url,
		postBack: "service-diagnosis",
		init: function(editor) {
			editor.Events.OnInitData.add(function(sender, data) {
				data.Columns
					.setprops("id", {label:"ID", numeric:true, key: true})
					.setprops("diagnosis_code", {label:"Diagnosis"})
					.setprops("condition", {label:"Condition"})
			...
			editor.Events.OnInitEditor.add(function(sender, editor) {
				editor.NewGroupEdit("General", function(editor, tab) {
					editor.AddGroup("Diagnosis in Layman's Term", function(editor) {
						// editor.AddLookup("address_type", {width:300, height:250, disableEdit:true, init:AddressTypesLookup});
						editor.AddMemo({ID: "condition", noLabel:true, height:50});
					...
					editor.initStatusToolbar = function() {
						
					};
					
					editor.AddGroup("Diagnosis Lookup", function(editor) {
						// editor.AddEdit({ID: "diagnosis_code", noLabel:true});
						editor.AddContainer({ID: "diagnosis_code", noLabel:true, noLabelPadding:true, noFocus:true, height:250, 
