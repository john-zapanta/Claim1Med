			...
			grid.Events.OnInitGrid.add(function(grid) {
				...
				grid.Events.OnInitData.add(function(grid, data) {
					data.Columns
						...
						.setprops("action", {label:"Description"})
				...
				grid.Events.OnInitColumns.add(function(grid) {
					...
					grid.NewTreeViewColumn({fname: "action", width: 450, fixedWidth:true}); //, float:"left"});
					grid.NewColumn({fname: "code", width: 100});
				...
				return {
					title: "Delete Action",
					message: ('Please confirm to delete action "<b>{0}</b>"').format(grid.dataset.lookup(id, "action"))
				}
			});
		}
	});	
